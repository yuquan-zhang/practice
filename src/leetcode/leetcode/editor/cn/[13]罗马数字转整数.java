...
class Solution13 {
    ...
    private final Map<String, Integer> romanIntMap = new HashMap<>();
    ...
    public int romanToInt(String s) {
        ...
        while (i < s.length()-1) {
            ...
            String twoKey = key +  s.charAt(i+1);
            if (romanIntMap.containsKey(twoKey)) {
                ans += romanIntMap.get(twoKey);
                i+=2;
            } else {
                ans += romanIntMap.get(key);
                i++;
            }
        }
        if (i < s.length()) {
            ans += romanIntMap.get("" + s.charAt(i));
        }
        return ans;
    }
}
